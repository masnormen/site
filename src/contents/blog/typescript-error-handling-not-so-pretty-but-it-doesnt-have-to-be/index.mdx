...
function generateData(str: string): DataJson {
...
export async function safe<T, E extends Error = Error>(fn: (...args: any[]) => T | Promise<T>): Promise<Result<T, E>> {
  try {
    const data = await fn();
    return [data, null];
  } catch (e) {
...
# Update as of 2024: ECMAScript _Safe Assignment Operator_
